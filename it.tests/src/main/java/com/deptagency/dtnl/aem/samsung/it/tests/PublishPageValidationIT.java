 ...
 */
package com.deptagency.dtnl.aem.adaptto.it.tests;
...
public class PublishPageValidationIT {
    ...
    private static void verifyLinkedResources(HtmlUnitClient client, String path) throws ClientException, IOException, URISyntaxException {
        ...
        assertFalse(path + " does not contain any references!", references.isEmpty());
        for (URI ref : references ) {
            if (isSameOrigin(client.getUrl(), ref)) {
                ...
                if (ZEROBYTEFILES.stream().noneMatch(s -> ref.getPath().startsWith(s)) && responseSize == 0 && LOG.isDebugEnabled()) {
                    LOG.info(String.format("Empty response body from [%s], please validate if this is correct", ref.getPath()));
                }
